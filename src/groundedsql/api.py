...
async function ask() {{
  ...
  try {{
    ...
    if (!r.ok) {{
      ...
      const gp = data.grounding_passed;
      const gi = data.grounding_interventions;
      out.innerHTML = `
        <div class="answer">${{escHtml(data.answer)}}</div>
        <div class="sql">${{escHtml(data.sql_used)}}</div>
        <div class="meta">
          <span class="pill">${{data.iterations}} iter</span>
          <span class="pill">${{data.elapsed_s}}s</span>
          <span class="pill">${{Math.round(data.confidence * 100)}}% confidence</span>
